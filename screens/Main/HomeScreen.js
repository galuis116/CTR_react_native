...
class HomeScreen extends Component {
    ...
    onPressShare = async () => {
        ...
        var message = `${months[this.props.user.month]} - ${this.props.user.date} \n${quote.title}\n${quote.quote}\n\n${quote.link}\n\n`;
    ...
    onChangeDate = date => {
        ...
        if( this.props.user.membership == "trial" && viewed_quotes.length > 13 && viewed_quotes.indexOf(documentID) == -1 ){
            Alert.alert(
                "CTR Daily",
        ...
        firestore().collection("quotes").doc(documentID).get()
            .then(snap => {
                const data = snap.data();
                const { viewed_quotes } = this.props.user;
                if (viewed_quotes.indexOf(documentID) == -1) {
                    firestore().collection('users').doc(this.props.user.uid).update({ viewed_quotes: firestore.FieldValue.arrayUnion(documentID) })
                    viewed_quotes.push(documentID);
                }
                const { loadUserData } = this.props;
                loadUserData({ quote: data, month, year, date: day, viewed_quotes });
                this.setState({ loading: false, month, year, date: day, currentDate: date });
            })
            .catch(e => {
                this.setState({ loading: false })
            })
    ...
    async componentDidMount() {
        if (this.props.user.membership == "trial" && this.props.user.membership_status == "active") {
            this._unsubscribe = this.props.navigation.addListener("focus", () => {
                const idOfQuote = `${months[this.props.user.month]}-${this.props.user.date}`;
                const { viewed_quotes } = this.props.user;
                if (viewed_quotes.indexOf(idOfQuote) == -1) {
                    viewed_quotes.push(idOfQuote);
                    var membership_status = "active";
                    if (viewed_quotes.length > 13) { membership_status = "expired"; }
                    firestore().collection('users').doc(this.props.user.uid).update({ viewed_quotes: firestore.FieldValue.arrayUnion(idOfQuote), membership_status })
                    const { loadUserData } = this.props;
        ...
        return (
            <View style={styles.container}>
                ...
                <ViewShot ref="viewShot" style={{ flex: 1 }}>
                    <FastImage source={{ uri: this.props.user.theme.background }} style={{ width: width, height: height, position: "absolute", }} />
                    <SafeAreaView>
                        ...
                        {this.props.user.quote && (
                            <View style={styles.quoteContainer}>
                                <View style={styles.date}>
                                    ...
                                    <Text style={dynamicStyles.month_year}>{months[this.props.user.month]}, {this.props.user.year}</Text>
                                </View>
                                <Text style={dynamicStyles.quoteTitle}>{this.props.user.quote.title}</Text>
                                <Text style={dynamicStyles.quote}>{this.props.user.quote.quote}</Text>
                                <TouchableOpacity style={dynamicStyles.link} onPress={() => this.onPressLink(this.props.user.quote.link)}>
                                    <Text style={dynamicStyles.linkText}>{this.props.user.quote.name}</Text>
                                </TouchableOpacity>
                            </View>
                        )}
                    </SafeAreaView>
                ...
                <View style={styles.footer}>
                    <TouchableOpacity style={styles.btn} onPress={() => this.props.navigation.navigate("Search")}>
                    ...
                    <TouchableOpacity style={styles.btn} onPress={() => navigation.navigate("Themes")}>
                    ...
                    <TouchableOpacity style={styles.btn} onPress={this.onPressFavorite}>
                        <Icon name={this.props.user.favorites.includes(`${months[this.props.user.month]}-${this.props.user.date}`) ? "heart" : "heart-outline"} size={34} color="white" />
                ...
                <View style={styles.header}>
                    ...
                    <TouchableOpacity style={[styles.btn, { marginLeft: 10 }]} onPress={this.onPressCalendar}>
