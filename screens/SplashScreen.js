...
class SplashScreen extends Component {
  ...
  createUser = async (uid) => {
    const user = {
      ...
      theme: {
        background:
          'https://firebasestorage.googleapis.com/v0/b/ctr-daily.appspot.com/o/themes%2FAngel%20Moroni.jpg?alt=media&token=1e6d65c0-27bd-4972-a731-f39016cfa733',
        color: 'white',
        name: 'Angel Moroni',
      },
      font: {
  ...
  LoadImages = async () => {
    var images = [],
      themes = [];
    // Load Themes
    const themeSnaps = await firestore()
      .collection('themes')
      .orderBy('order')
      .get();
    ...
    messaging()
      ...
      .then((token) => {
        firestore()
          .collection('users')
          .doc(uid)
          .update({pushToken: firestore.FieldValue.arrayUnion(token)});
      });
    if (user.showWelcome) {
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <WelcomeModal
          isVisible={this.state.showWelcomeModal}
